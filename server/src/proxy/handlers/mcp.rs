...
async fn handle_vision_get(state: Arc<AppState>, headers: HeaderMap) -> Response {
...
async fn handle_vision_delete(state: Arc<AppState>, headers: HeaderMap) -> Response {
    let Some(session_id) = mcp_session_id(&headers) else {
        return (StatusCode::BAD_REQUEST, "Missing Mcp-Session-Id").into_response();
    };
...
async fn handle_vision_post(state: Arc<AppState>, headers: HeaderMap, body: Body) -> Response {
    let collected = match to_bytes(body, 100 * 1024 * 1024).await {
        Ok(b) => b,
        Err(e) => {
            return (
