...
pub fn wrap_request(body: &Value, project_id: &str, mapped_model: &str) -> Value {
    ...
    if let Some(image_config) = config.image_config {
        ...
        // [NEW] 只在非图像生成模式下注入 Antigravity 身份 (原始简化版 - 与 Main 分支一致)
    ...
    // NOTE: thinkingConfig is NOT removed here - it's needed for Gemini 3 Pro models
    if let Some(obj) = inner_request.as_object_mut() {
        let gen_config = obj.entry("generationConfig").or_insert_with(|| json!({}));
        if let Some(gen_obj) = gen_config.as_object_mut() {
            // Only clean fields that cause conflicts, NOT thinkingConfig
            gen_obj.remove("responseMimeType");
