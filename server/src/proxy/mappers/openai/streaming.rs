                        ...
                        if let Ok(line_str) = std::str::from_utf8(&line_raw) {
                            ...
                            if line.starts_with("data: ") {
                                ...
                                if let Ok(mut json) = serde_json::from_str::<Value>(json_part) {
                                    ...
                                    // [NEW] Extract usageMetadata for token counting when finishReason is present
                                    if finish_reason.is_some() {
                                        if let Some(usage) = actual_data.get("usageMetadata") {
                                            ...
                                            total_tokens_out = usage.get("candidatesTokenCount")
                                                ...
                                                .unwrap_or(0) as u32;
                                            tracing::info!(
                                                "[OpenAI-Stream] ✓ Stream completed | In: {} tokens | Out: {} tokens",
