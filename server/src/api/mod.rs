...
pub fn build_routes(state: Arc<AppState>) -> Router {
    Router::new()
        // Account
        .route("/api/accounts", get(account::list_accounts).post(account::add_account))
        .route("/api/accounts/:id", axum::routing::delete(account::delete_account))
        ...
        .route("/api/accounts/quota/refresh/:id", get(account::refresh_account_quota))
        .route("/api/accounts/quota/refresh", post(account::refresh_all_quotas))
        
        // Import
        .route("/api/import/json", post(import::import_accounts_json))
        
        // Config
        .route("/api/config", get(config::load_config).put(config::save_config))
