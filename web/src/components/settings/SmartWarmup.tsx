...
const SmartWarmup = ({ config, onChange }: SmartWarmupProps) => {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              {t(
                "settings.warmup.desc",
                "自动预热账号配额，避免长时间闲置导致配额冻结"
              )}
            </p>
          </div>
        </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {warmupModelsOptions.map((model) => {
                  const isSelected = config.monitored_models?.includes(
                    model.id
                  );
                  return (
                    <div
              ...
              <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-2 leading-relaxed">
                {t(
                  "settings.warmup.monitored_desc",
                  "勾选需要预热的模型。当选中的模型配额达到 100% 时，将自动触发预热"
                )}
              </p>
            </div>
          </div>
