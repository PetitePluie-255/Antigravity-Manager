...
const QuotaProtection = ({ config, onChange }: QuotaProtectionProps) => {
  ...
  const handleEnabledChange = (enabled: boolean) => {
    ...
    if (
      enabled &&
      (!config.monitored_models || config.monitored_models.length === 0)
    ) {
      newConfig.monitored_models = monitoredModelsOptions.map((o) => o.id);
    }
    onChange(newConfig);
  };
  ...
  const handlePercentageChange = (value: string) => {
    ...
    const clampedPercentage = Math.min(99, Math.max(1, parsed));
  ...
  const exampleThreshold = Math.floor(
    (exampleTotal * config.threshold_percentage) / 100
  );
    ...
    <div className="animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-rose-50 dark:bg-rose-900/20 flex items-center justify-center text-rose-500 transition-all duration-300">
            <Shield size={20} />
          </div>
          <div>
            <div className="font-bold text-gray-900 dark:text-gray-100">
              {t("settings.quota_protection.title", "配额保护")}
            </div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              {t(
                "settings.quota_protection.enable_desc",
                "当任一监控模型配额低于阈值时，暂停该账号以保护配额"
              )}
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 pt-5 border-t border-gray-100 dark:border-base-200 space-y-6 animate-in slide-in-from-top-1 duration-200">
          ...
          <div className="flex items-center gap-4">
            <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
              {t("settings.quota_protection.threshold_label", "保护阈值")}
            </label>
          ...
          <div className="space-y-3">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                {t(
                  "settings.quota_protection.monitored_models_label",
                  "监控模型"
                )}
              </label>
              <p className="text-[10px] text-gray-400 dark:text-gray-500">
                {t(
                  "settings.quota_protection.monitored_models_desc",
                  "勾选需要监控的模型。当选中的任一模型利用率跌破阈值时，将触发保护"
                )}
              </p>
            </div>
                      ...
                      className={`
                                            ...
                                            ${
                                              isSelected
                                                ...
                                                : "bg-gray-200 dark:bg-base-300 text-transparent scale-75 opacity-0"
                                            }
                                        `}
                    >
            ...
            <div className="flex flex-col gap-1">
              <p className="text-xs text-blue-700/80 dark:text-gray-300/90 leading-relaxed font-medium">
                {t("settings.quota_protection.example", {
                  ...
                  `当配额低于 ${config.threshold_percentage}% 时（如总配额 ${exampleTotal} 次中剩余 ${exampleThreshold} 次以下），账号将被暂停`}
              </p>
              <span className="block font-bold text-emerald-600 dark:text-emerald-400 text-[10px] uppercase tracking-wide">
                ✓{" "}
                {t(
                  "settings.quota_protection.auto_restore_info",
                  "配额恢复后自动重新启用"
                )}
              </span>
            </div>
