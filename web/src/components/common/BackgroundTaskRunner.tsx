...
function BackgroundTaskRunner() {
  ...
  useEffect(() => {
    ...
    if (auto_refresh && !prevAutoRefreshRef.current) {
      console.log(
        "[BackgroundTask] Auto-refresh enabled, executing immediately..."
      );
      refreshAllQuotas();
    }
    ...
    if (auto_refresh && refresh_interval > 0) {
      console.log(
        `[BackgroundTask] Starting auto-refresh quota timer: ${refresh_interval} mins`
      );
  ...
  useEffect(() => {
    ...
    const { fetchCurrentAccount } = useAccountStore.getState();

    // Check if we just turned it on
    if (auto_sync && !prevAutoSyncRef.current) {
      console.log(
        "[BackgroundTask] Auto-sync enabled, executing immediately..."
      );
      fetchCurrentAccount();
    }
    ...
    if (auto_sync && sync_interval > 0) {
      console.log(
        `[BackgroundTask] Starting auto-sync account timer: ${sync_interval} seconds`
      );
      intervalId = setInterval(() => {
