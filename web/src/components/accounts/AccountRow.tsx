...
function AccountRow({
  ...
  const geminiProModel = account.quota?.models.find(
    (m) => m.name.toLowerCase() === "gemini-3-pro-high"
  );
  const geminiFlashModel = account.quota?.models.find(
    (m) => m.name.toLowerCase() === "gemini-3-flash"
  );
  const geminiImageModel = account.quota?.models.find(
    (m) => m.name.toLowerCase() === "gemini-3-pro-image"
  );
  const claudeModel = account.quota?.models.find(
    (m) => m.name.toLowerCase() === "claude-sonnet-4-5-thinking"
  );
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            {account.quota?.is_forbidden && (
              <span
                className="px-2 py-0.5 rounded-md bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400 text-[10px] font-bold flex items-center gap-1 shadow-sm border border-red-200/50"
                title={t("accounts.forbidden_tooltip")}
              >
        ...
        {account.quota?.is_forbidden ? (
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 py-0">
            ...
            <div className="relative h-[22px] flex items-center px-1.5 rounded-md overflow-hidden border border-gray-100/50 dark:border-white/5 bg-gray-50/30 dark:bg-white/5 group/quota">
              {geminiProModel && (
                <div
                  className={`absolute inset-y-0 left-0 transition-all duration-700 ease-out opacity-15 dark:opacity-20 ${getColorClass(
              ...
              <div className="relative z-10 w-full flex items-center text-[10px] font-mono leading-none">
                <span
                  className="w-[54px] text-gray-500 dark:text-gray-400 font-bold truncate pr-1"
                  title="Gemini 3 Pro"
                >
                  G3 Pro
                </span>
                  ...
                  {geminiProModel?.reset_time ? (
                    <span
                      className={cn(
                        ...
                        getTimeColorClass(geminiProModel.reset_time)
                      )}
                    >
                    ...
                    <span className="text-gray-300 dark:text-gray-600 italic scale-90">
                      N/A
                    </span>
                  )}
                </div>
                <span
                  className={cn(
                    ...
                    getQuotaColor(geminiProModel?.percentage || 0) === "success"
                      ? "text-emerald-600 dark:text-emerald-400"
                      : getQuotaColor(geminiProModel?.percentage || 0) ===
                        "warning"
            ...
            <div className="relative h-[22px] flex items-center px-1.5 rounded-md overflow-hidden border border-gray-100/50 dark:border-white/5 bg-gray-50/30 dark:bg-white/5 group/quota">
              ...
              <div className="relative z-10 w-full flex items-center text-[10px] font-mono leading-none">
                ...
                <div className="flex-1 flex justify-center">
                  {geminiFlashModel?.reset_time ? (
                    <span
                      className={cn(
                        "flex items-center gap-0.5 font-medium transition-colors",
                        getTimeColorClass(geminiFlashModel.reset_time)
                      )}
                    >
                      <Clock className="w-2.5 h-2.5" />
                ...
                <span
                  className={cn(
                    "w-[36px] text-right font-bold transition-colors",
                    getQuotaColor(geminiFlashModel?.percentage || 0) ===
                      "success"
                      ? "text-emerald-600 dark:text-emerald-400"
                      : getQuotaColor(geminiFlashModel?.percentage || 0) ===
                        "warning"
            ...
            <div className="relative h-[22px] flex items-center px-1.5 rounded-md overflow-hidden border border-gray-100/50 dark:border-white/5 bg-gray-50/30 dark:bg-white/5 group/quota">
              ...
              <div className="relative z-10 w-full flex items-center text-[10px] font-mono leading-none">
                ...
                <div className="flex-1 flex justify-center">
                  {geminiImageModel?.reset_time ? (
                    <span
                      className={cn(
                        "flex items-center gap-0.5 font-medium transition-colors",
                        getTimeColorClass(geminiImageModel.reset_time)
                      )}
                    >
                      <Clock className="w-2.5 h-2.5" />
                ...
                <span
                  className={cn(
                    "w-[36px] text-right font-bold transition-colors",
                    getQuotaColor(geminiImageModel?.percentage || 0) ===
                      "success"
                      ? "text-emerald-600 dark:text-emerald-400"
                      : getQuotaColor(geminiImageModel?.percentage || 0) ===
                        "warning"
            ...
            <div className="relative h-[22px] flex items-center px-1.5 rounded-md overflow-hidden border border-gray-100/50 dark:border-white/5 bg-gray-50/30 dark:bg-white/5 group/quota">
              ...
              <div className="relative z-10 w-full flex items-center text-[10px] font-mono leading-none">
                ...
                <div className="flex-1 flex justify-center">
                  {claudeModel?.reset_time ? (
                    <span
                      className={cn(
                        "flex items-center gap-0.5 font-medium transition-colors",
                        getTimeColorClass(claudeModel.reset_time)
                      )}
                    >
                      <Clock className="w-2.5 h-2.5" />
                ...
                <span
                  className={cn(
                    "w-[36px] text-right font-bold transition-colors",
                    getQuotaColor(claudeModel?.percentage || 0) === "success"
                      ? "text-emerald-600 dark:text-emerald-400"
                      : getQuotaColor(claudeModel?.percentage || 0) ===
                        "warning"
                      ? "text-amber-600 dark:text-amber-400"
                      : "text-rose-600 dark:text-rose-400"
                  )}
      ...
      <td className="px-4 py-1">
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-gray-400 dark:text-gray-500 font-mono whitespace-nowrap leading-tight">
            {new Date(account.last_used * 1000).toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            })}
          </span>
      ...
      <td className="px-4 py-1">
        <div className="flex items-center gap-0.5 opacity-60 group-hover:opacity-100 transition-opacity">
          ...
          {onDeviceManage && (
            <button
              className="p-1.5 text-gray-500 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/30 rounded-lg transition-all"
              onClick={(e) => {
                e.stopPropagation();
                onDeviceManage();
              }}
              title={t("device.dialog_title", "设备指纹管理")}
            >
              <Fingerprint className="w-3.5 h-3.5" />
            </button>
          )}
          <button
            className={`p-1.5 text-gray-500 dark:text-gray-400 rounded-lg transition-all ${
              isSwitching
                ? "bg-blue-50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400 cursor-not-allowed"
          ...
          >
            <ArrowRightLeft
              className={`w-3.5 h-3.5 ${isSwitching ? "animate-spin" : ""}`}
            />
          </button>
          <button
            className={`p-1.5 text-gray-500 dark:text-gray-400 rounded-lg transition-all ${
              isRefreshing
                ? "bg-green-50 dark:bg-green-900/10 text-green-600 dark:text-green-400 cursor-not-allowed"
