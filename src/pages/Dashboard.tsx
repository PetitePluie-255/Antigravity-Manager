...
function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const claudeQuotas = accounts
      .map(
        (a) =>
          a.quota?.models.find(
            (m) => m.name.toLowerCase() === "claude-sonnet-4-5"
          )?.percentage || 0
      )
      .filter((q) => q > 0);
    ...
    return {
      ...
      avgGemini:
        geminiQuotas.length > 0
          ? Math.round(
              geminiQuotas.reduce((a, b) => a + b, 0) / geminiQuotas.length
            )
          : 0,
      avgGeminiImage:
        geminiImageQuotas.length > 0
          ? Math.round(
              geminiImageQuotas.reduce((a, b) => a + b, 0) /
                geminiImageQuotas.length
            )
          : 0,
      avgClaude:
        claudeQuotas.length > 0
          ? Math.round(
              claudeQuotas.reduce((a, b) => a + b, 0) / claudeQuotas.length
            )
          : 0,
  ...
  const exportAccountsToJson = async (accountsToExport: Account[]) => {
    try {
      ...
      const fileName = `antigravity_accounts_${
      ...
      if (isTauri()) {
        ...
        if (!path) return;
        await apiCall("save_text_file", { path, content });
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <div className="bg-white dark:bg-base-100 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-base-200">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400">
              {t("dashboard.avg_gemini_image")}
            </div>
            {stats.avgGeminiImage > 0 && (
              <div
                className={`text-[10px] mt-1 ${
                  stats.avgGeminiImage >= 50
                    ? "text-green-600 dark:text-green-400"
                    : "text-orange-600 dark:text-orange-400"
                }`}
              >
