...
function Settings() {
  ...
  const handleDetectAntigravityPath = async () => {
    try {
      const path = await invoke<string>("get_antigravity_path", {
        bypassConfig: true,
      });
  ...
  const handleCheckUpdate = async () => {
    ...
    try {
      ...
      showToast(
        `${t("settings.about.update_check_failed")}: ${error}`,
        "error"
      );
    } finally {
              ...
              <div>
                ...
                >
                  <option value="disabled">
                    {t("settings.general.auto_launch_disabled")}
                  </option>
                  <option value="enabled">
                    {t("settings.general.auto_launch_enabled")}
                  </option>
                </select>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                  {t("settings.general.auto_launch_desc")}
                </p>
              </div>
            </div>
          )}
          ...
          {activeTab === "account" && (
            <div className="space-y-6">
              ...
              {formData.auto_sync && (
                <div className="ml-4">
                  <label className="block text-sm font-medium text-gray-900 dark:text-base-content mb-2">
                    {t("settings.account.sync_interval")}
                  </label>
                  <input
                    type="number"
                    className="w-32 px-4 py-4 border border-gray-200 dark:border-base-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 dark:text-base-content bg-gray-50 dark:bg-base-200"
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder={t(
                      "settings.advanced.antigravity_path_placeholder"
                    )}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        antigravity_executable: e.target.value,
                      })
                    }
                  />
                  {formData.antigravity_executable && (
                    <button
                      ...
                      onClick={() =>
                        setFormData({
                          ...formData,
                          antigravity_executable: undefined,
                        })
                      }
                    >
                      {t("common.clear")}
