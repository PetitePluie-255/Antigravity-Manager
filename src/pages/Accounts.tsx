...
function Accounts() {
  ...
  const ITEMS_PER_PAGE = useMemo(() => {
    ...
    if (viewMode === "list") {
      ...
      return Math.max(
        1,
        Math.floor((containerSize.height - headerHeight) / rowHeight)
      );
    } else {
  ...
  const filteredAccounts = useMemo(() => {
    ...
    if (filter === "available") {
      result = result.filter((a) => {
        const gemini =
          a.quota?.models.find((m) => m.name.toLowerCase().includes("gemini"))
            ?.percentage || 0;
        const claude =
          a.quota?.models.find((m) => m.name.toLowerCase().includes("claude"))
            ?.percentage || 0;
    ...
    <div className="h-full flex flex-col p-5 gap-4 max-w-7xl mx-auto w-full">
      ...
      <div className="flex-none flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            className={`px-3 py-2 bg-blue-500 text-white text-xs font-medium rounded-lg hover:bg-blue-600 transition-colors flex items-center gap-1.5 shadow-sm ${
