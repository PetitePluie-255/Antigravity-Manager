...
function AddAccountDialog({ onAdd }: AddAccountDialogProps) {
  ...
  const [activeTab, setActiveTab] = useState<"oauth" | "token" | "import">(
    "oauth"
  );
  ...
  const { startOAuthLogin, cancelOAuthLogin, importFromDb, importV1Accounts } =
    useAccountStore();
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      ...
      try {
        ...
        console.warn("Failed to listen to tauri event:", e);
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      setMessage(
        t("accounts.add.token.batch_progress", {
          current: i + 1,
          total: tokens.length,
        })
      );
    ...
    if (successCount === tokens.length) {
      ...
      setMessage(
        t("accounts.add.token.batch_partial", {
  ...
  const handleFileUpload = async (
    ...
    const actionName = t("accounts.add.tabs.import");
    setStatus("loading");
    setMessage(`${actionName}...`);
    ...
    try {
      ...
      setStatus("error");
      setMessage(`${actionName} ${t("common.error")}: ${String(error)}`);
    }
        ...
        createPortal(
          <dialog className="modal modal-open z-[100]">
            ...
            <div
              ...
              className="fixed top-0 left-0 right-0 h-8 z-[110]"
            />
                ...
                {activeTab === "oauth" && (
                  <div className="space-y-6 py-4">
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        {status === "loading"
                          ? t("accounts.add.oauth.btn_waiting")
                          : t("accounts.add.oauth.btn_start")}
                      </button>
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        {isTauri()
                          ...
                          : t("accounts.add.import.btn_file") ||
                            "选择文件导入 (JSON)"}
                      </button>
                    </div>

                    <div className="divider text-xs text-gray-300 dark:text-gray-600">
                      {t("accounts.add.import.or")}
                    </div>
